...
func TargetStep(option *pb.Option) *pb.StepSummary {
	files := TargetFile(option)
	return CountStep(files)
}

// Step 统计多个代码文件的代码行数
func CountStep(file []string) *pb.StepSummary {

	// 读取注释定义
	var commentDef []CommentDefine
	err := YamlToStruct(DefineFile, &commentDef)
	if err != nil {
	...
	// 汇总
	return Summary(stepInfo)
}

// Summary 代码行数统计结果
func Summary(stepInfo []pb.StepInfo) *pb.StepSummary {
	// 代码行数统计信息汇总
	var stepSummary pb.StepSummary

	// 合计
	for i, info := range stepInfo {
		// 总行数
		stepSummary.TotalStep = stepSummary.TotalStep + info.TotalStep
		// 空行总行数
		stepSummary.EmptyLineStep = stepSummary.EmptyLineStep + info.EmptyLineStep
		// 注释总行数
		stepSummary.CommentStep = stepSummary.CommentStep + info.CommentStep
		// 代码总行数
		stepSummary.SourceStep = stepSummary.SourceStep + info.SourceStep
		// 有效总行数(注释+代码)
		stepSummary.ValidStep = stepSummary.ValidStep + info.ValidStep

		// 无注释定义
		if !info.CommentDefined {
			stepSummary.FlatFile = append(stepSummary.FlatFile, info.File)
		}
		// 未统计文件
		if !info.Counted {
			stepSummary.UnCountedFile = append(stepSummary.UnCountedFile, info.File)
		}
...
func CountAll(file []string, mCommentRegExp *map[string]CommentRegExp) []pb.StepInfo {
	var stepInfo []pb.StepInfo
	for _, f := range file {
...
// Count 统计单个文件的代码行数
func Count(file string, mCommentRegExp *map[string]CommentRegExp, infoAll *[]pb.StepInfo) {
	// 获取代码文件对应的注释定义
...
// 统计
func count(cmtRegExp *CommentRegExp, info *pb.StepInfo, infoAll *[]pb.StepInfo) {
	var totalStep int32     // 总行数
	var emptyLineStep int32 // 空行数
	var commentStep int32   // 注释
	var sourceStep int32    // 代码行数
